...
(
    ...
    expected_people_default INTEGER CHECK (expected_people_default IS NULL OR expected_people_default >= 0),
...
CREATE TABLE bridge_event_zone_day
(
    ...
    impact_weight      NUMERIC(6, 3) CHECK (impact_weight IS NULL OR impact_weight >= 0),

    CONSTRAINT fk_bridge_event_zone_day_event
        FOREIGN KEY (event_key) REFERENCES dim_event (event_key),
    CONSTRAINT fk_bridge_event_zone_day_zone
        FOREIGN KEY (zone_key) REFERENCES dim_zone (zone_key),
    CONSTRAINT fk_bridge_event_zone_day_date
        FOREIGN KEY (date_key) REFERENCES dim_date (date_key),

    CONSTRAINT uq_bridge_event_zone_day UNIQUE (event_key, zone_key, date_key)
);
...
CREATE INDEX idx_bridge_event_zone_day_date_key
    ON bridge_event_zone_day (date_key);

CREATE INDEX idx_bridge_event_zone_day_zone_key
    ON bridge_event_zone_day (zone_key);
